   	...
   	var App=function(){
	      ...
	      this.url={
	      	cdnUrl:cdnurl,
	        	...
	        	addAcount:weburl+"/partnerManagement/addAccount.html?type=2&pageTimeStap="+pageTimeStap,//添加账户  0 渠道 1 物流 2 车商
	        	editAcount:weburl+"/partnerManagement/sessionEditBankAccount.html?type=2&pageTimeStap="+pageTimeStap,//修改账户 0 渠道 1 物流 2 车商
	        	delAcount:weburl+"/partnerManagement/sessionDeleteBankAccount.html?type=2&pageTimeStap="+pageTimeStap,//删除缓存账号
	        	stopAcount:weburl+"/partnerManagement/sessionUpdateStatusBankAccount.html?type=2&pageTimeStap="+pageTimeStap,//修改状态
	        	lookAcount:weburl+"/partnerManagement/sessionGetBankAccount.html?type=2&pageTimeStap="+pageTimeStap,//查看账户
	      ...
	      initDateAndCity:function(){
	        ...
	        laydate(end);
	     	...
	     	initImgUpload:function(){
	      	/*注意 
	      		1.没个区域的可上传图片数量不一样
			      2.删除老照片和新照片的处理不一样  新照片：前端删除div即可   老照片：需要在页面的form中添加删除图片的隐藏域信息
	      	*/
	      	...
	      	function createConfig(type){// 0 营业执照 1 营业场所 2 合作协议 3 法人身份证 4 企业信用报告  type会传入隐藏域   后端根据type判定上图片类别
      		...
      		require.async("uploadify",function(){
      			for(var i=0;i<5;i++){//0 营业执照 1 营业场所 2 合作协议 3 法人身份证 4企业信用报告
      				var uploadId="#Upload"+i;
      				var preId="#PreWraper"+i;
      				$(uploadId).uploadify(createConfig(i));
      				if($(preId).find(".image-item").length>=imgLimit){
      					$(uploadId).addClass("hide");
      				}
      			}
	      	});
	      	...
	      	$("body").on("click",".file-del",function(){//删除图片   设置最大可上传图片的数量
	      		var $del=$(this);
	      		...
	      		if($del.hasClass("old")){//如果是删除老照片 需要添加删除的图片信息隐藏域
	      			var id=$del.attr("data-id");
	      			var type=$del.attr("data-type");
	      			var $input=$("<input type='hidden' />");
	      			$input.attr("name","del_"+type+"_"+id).attr("value",id);
	      			$("#deleteImgWraper").append($input);
	      		}
	      	})
	      },
	      addAcount:function(){
	      	...
	      	$("#addCount").on("click",function(){
	      		if($("#type option:selected").val()==0){
	      			var url=self.url.addAcount+"&removeGongGuan=1";//需要移除弹框里面的共管户
	      		}
	      		else{
	      			var url=self.url.addAcount;
	      		}
	      		parent.layer.open({
		            type: 2,
		            title: '添加账号',
			         ...
			         require.async("jqueryValidateAdditional",function(){
			            ...
			            self.validater=$("#pageForm").validate({
			               ...
			               //  	self.validater.element(inp);
							...
							$("#type").on("change",function(){//根据车商来源动态更改渠道商名称的验证配置
								var $select=$(this);
								...
								if(value==1){//来源渠道
									...
									$("#sourcesId").rules("add",{
										selectArea:true,
										messages:{selectArea:"请选择渠道商名称"}
									});
								}
								else{
									$(".qudao-item").addClass("hide");
									$("#sourcesId").rules("remove"); 
								}
							});
