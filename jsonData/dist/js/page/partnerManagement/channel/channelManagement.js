...
define(function(require,exports,module){
  ...
  template.helper("qudaoStatus",function(status){//启用状态
    return status==0?"停用":"正常";
  });

  template.helper("qudaoStatusBtn",function(status){//启用状态按钮
  ...
  $(function(){
    ...
    var App=function(){
      ...
      this.url={
        webUrl:weburl,
        listUrl:weburl+"/partnerManagement/getSourceList.html",//渠道列表
        switchQudao:weburl+"/partnerManagement/updateSourcesRegisterStatus.html",//更改渠道状态
        delQudao:weburl+"/partnerManagement/deleteSources.html"//删除渠道
      };

      this.tpl={
        qudaoTpl:template.compile($("#qudaoTpl").html())
      };

      this.parentIframe=$(parent.document.getElementById("iframePage")).eq(0);
      this.getQueryData=function(){//获取查询参数
        var $qudaoInp=$("#qudaoInp");
        var $hezuoSelect=$("#hezuoSelect");

        var qudao=$qudaoInp.val();
        var hezuoStatus=$hezuoSelect.find("option:selected").attr("value");
        var param={"companyName":qudao,"cooperationStatus":hezuoStatus};
        return param;
      };

      this.loadList=function(query){//根据不同的请求参数加载不同的查询列表
        var self=this;
        var $qudaoTable=$("#qudaoTable");
        var $qudaoTbody=$("#qudaoBody");
        var pageSize=10;
          ...
          success:function(data){
            if(data.result){
              if(data.pages&&data.pages.total > 0){
                var html=self.tpl.qudaoTpl(data);
                $qudaoTbody.html(html);
                $qudaoTable.removeClass("hide");
    ...
    App.prototype={
      ...
      operateTable:function(){
        ...
        $("#qudaoTable").on("click",".operate-td .stop",function(){//停用 启用
          var $btn=$(this);
          ...
          var $btn=$(this);
          var id=$btn.attr("data-id");
          parent.layer.confirm('是否删除该渠道?', 
            {btn: ['确认','取消']}, //按钮
            function(){
              $.ajax({
                url:self.url.delQudao+"?id="+id,
                type:"get",
                dataType:"json",
                success:function(data){
                  if(data.result){
